                                 ...
                                 uint8_t bits, int bound = B_DCT2)
{
    ...
    DLTensor tx  = make_cpu_tensor(x.data(),     fshape, fstr, bits);
    DLTensor tw  = make_cpu_tensor(w.data(),     fshape, fstr, bits);  // wc==nc -> RLS (genuine per-channel)
    DLTensor tLx = make_cpu_tensor(Lx.data(),    fshape, fstr, bits);
    ...
    for (int64_t c = 0; c < C; ++c) {
        ...
        DLTensor twc = make_cpu_tensor(wc_.data(), sshape, sstr, bits);  // wc==1 -> JRLS (broadcast)
