...
// field_matvec_rls / field_diag_rls: JRLS (wc == C) with a *genuine*
// per-channel weight map -- a ground-truth regression test for
// fastfields-cpu-lib#65. The `_jrls` kernels used to read the weight value
// once per voxel, outside any per-channel loop, and silently broadcast
// channel 0's weight to every channel; every prior RLS/JRLS test here uses
// either an all-ones weight (invisible to a broadcast bug: every channel's
// "true" weight is the same 1.0 anyway) or only checks self-adjointness
// (symmetric regardless of which value gets broadcast, as long as it is
// applied consistently forward and adjoint) -- neither catches this.
//
...
// the RLS (wc == 1) path instead -- a *different* dispatch branch
// (matvec_*_rls, not matvec_*_jrls; `field_rls_is_jrls` selects RLS whenever
// wc == 1, irrespective of nc) that is already extensively verified above.
// This is therefore not a self-consistency check against the (potentially
// buggy) JRLS code: it is an independent ground truth from a separately
// dispatched, separately tested code path. Channel weights are given
// distinct per-channel frequency and phase so that w(...,c) != w(...,c')
// at every voxel, which is exactly what makes a channel-0 broadcast visible
// for every c > 0.
template <typename scalar_t>
                                 ...
                                 uint8_t bits, int bound = B_DCT2)
{
    ...
    for (int64_t i = 0; i < snum; ++i)
        for (int64_t c = 0; c < C; ++c) {
            ...
            w[i * C + c] = (scalar_t)(0.5 + std::fabs(
                std::sin(0.19 * i * (c + 1) + 0.9 * (c + 1) + 0.4)));
        }
    ...
    DLTensor tx  = make_cpu_tensor(x.data(),     fshape, fstr, bits);
    DLTensor tw  = make_cpu_tensor(w.data(),     fshape, fstr, bits);  // wc==C -> JRLS
    DLTensor tLx = make_cpu_tensor(Lx.data(),    fshape, fstr, bits);
    ...
    for (int64_t c = 0; c < C; ++c) {
        ...
        DLTensor twc = make_cpu_tensor(wc_.data(), sshape, sstr, bits);  // wc==1 -> RLS
        DLTensor tLc = make_cpu_tensor(Lc.data(),  sshape, sstr, bits);
        DLTensor tdc = make_cpu_tensor(dc.data(),  sshape, sstr, bits);
        ff::cpu::field_matvec_rls(tLc, txc, twc, nullptr, ac.data(), mcp, bcp, (int8_t)bound, 2, 0);
        ff::cpu::field_diag_rls(tdc, twc, nullptr, ac.data(), mcp, bcp, (int8_t)bound, 2, 0);

        char bufm[160], bufd[160];
        std::snprintf(bufm, sizeof(bufm),
            "field2d_matvec_jrls_per_channel[C=%lld c=%lld order=%d bnd=%d]",
            (long long)C, (long long)c, order, bound);
        std::snprintf(bufd, sizeof(bufd),
...
int main()
{
    ...
    // JRLS (wc == C) with a *genuine* per-channel weight map: ground-truth
    // regression test for fastfields-cpu-lib#65 (the _jrls kernels used to
    // broadcast channel 0's weight to every channel). See the oracle's own
    // doc comment for why decomposing into single-channel RLS problems is an
    // independent ground truth rather than a self-consistency check.
    for (int bnd : {B_ZERO, B_DCT2, B_DST2, B_DFT}) {
        run_2d_rls_jrls_per_channel<double>(6, 7, 3, 1, {1.75, 0.9, 1.3}, {0, 0, 0}, {0, 0, 0}, 64, bnd);
        run_2d_rls_jrls_per_channel<double>(6, 7, 2, 2, {0.3, 0.4}, {1.0, 0.7}, {0, 0}, 64, bnd);
        run_2d_rls_jrls_per_channel<double>(7, 8, 3, 2, {0.3, 0.4, 0.2}, {1.0, 0.7, 1.2}, {0, 0, 0}, 64, bnd);
    }
    ...
    for (int bnd : {B_DCT2, B_DST2, B_DFT}) {
        run_2d_rls_jrls_per_channel<double>(7, 8, 2, 3, {0.3, 0.4}, {1.0, 0.7}, {1.1, 0.9}, 64, bnd);
        run_2d_rls_jrls_per_channel<double>(7, 8, 3, 3, {0.3, 0.4, 0.5}, {1.0, 0.7, 0.6}, {1.1, 0.9, 0.8}, 64, bnd);
    }
    run_2d_rls_jrls_per_channel<float>(6, 6, 3, 2, {0.3, 0.4, 0.2}, {1.0, 0.7, 1.2}, {0, 0, 0}, 32);
